...
void sendPIReventToMQTT(const char* roomID) {
  time_t now = 0;
  ...
  int size = snprintf(msg, sizeof(msg), 
    "{\"sensors\":[{\"name\":\"PIR\",\"values\":[{\"timestamp\":%llu, \"roomID\":\"%s\"}]}]}", now * 1000, roomID);
...
void sendPIR_BEDeventToMQTT(const char* roomID) {
  ...
  int size = snprintf(msg, sizeof(msg), 
    "{\"sensors\":[{\"name\":\"PIR_BED\",\"values\":[{\"timestamp\":%llu, \"roomID\":\"%s\"}]}]}", now * 1000, roomID);
  ESP_LOGI("mqtt", "Sent <%s> to topic %s", msg, device_topic);
  auto err = esp_mqtt_client_publish(mqtt_client, device_topic, msg, size, 1, 0);
  if (err == -1) {
    printf("Error while publishing to mqtt\n");
...
void sendBatteryStatusToMQTT(void) {
  ...
  int size = snprintf(msg, sizeof(msg), 
    "{\"sensors\":[{\"name\":\"battery\",\"values\":[{\"timestamp\":%llu, \"voltage\":%.1f, \"soc\":%.1f}]}]}", now * 1000, voltage, rsoc);
  ESP_LOGI("mqtt", "Sent <%s> to topic %s", msg, device_topic);
  auto err = esp_mqtt_client_publish(mqtt_client, device_topic, msg, size, 1, 0);
  if (err == -1) {
    printf("Error while publishing to mqtt\n");
...
void sendDOOReventToMQTT(void) {
  ...
  int size = snprintf(msg, sizeof(msg), 
    "{\"sensors\":[{\"name\":\"door\",\"values\":[{\"timestamp\":%llu, \"state\":\"open\"}]}]}", now * 1000);
  ESP_LOGI("mqtt", "Sent <%s> to topic %s", msg, device_topic);
  auto err = esp_mqtt_client_publish(mqtt_client, device_topic, msg, size, 1, 0);
  if (err == -1) {
    printf("Error while publishing to mqtt\n");
